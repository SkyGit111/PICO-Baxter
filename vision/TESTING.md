...
本轮低延迟优化后的默认日志应显示 `bitrate=6.0Mbps GOP=15 VBV=50ms`。请用手机
秒表或可同时拍到 D455 视野与 PICO 画面的高帧率相机测量延迟，不要只记录主观
体感。至少记录 30 个样本的中位数、P95 和最大值，并同步保存发送端每 5 秒的
`pipeline-age`、`send`、`max`、`slow-sends`、`reconnects`、`gaps`。

如果仍有明显延迟或抖动，不要盲目增大 socket 缓冲区。按
[`LATENCY_TUNING.md`](LATENCY_TUNING.md) 的判读顺序区分主机编码积压、TCP/Wi-Fi
背压和 PICO 解码/渲染积压，再运行其中的激进降载配置。
...
验收建议：连续 30 分钟内控制链不能因视频故障退出；视频应保持接近 30 AU/s；
不应持续出现 `slow-sends`、`reconnects` 或 `gaps`。端到端延迟阈值需要结合实验
任务确定，在完成真机测量前不得将自动化测试结果写成低延迟真机验收通过。
